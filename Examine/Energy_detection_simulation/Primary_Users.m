...
clc 
%rand('twister',1);
blockpu=[]; %primary user
blocksu=[]; %secondary user
for N=3:2:7
    block=[];
...
len_X = length(member); %总主用户数 
len_CR = length(member_CR); %总次用户
%***************************************** 
%输出结果 
%***************************************** 
...
figure(1);
subplot(2,1,1);
plot([0 tp(1,member)],0:len_X); %member 1-100
%绘出各主用户的到达时间图;
hold on;
...
figure(3);
lambdap =0.01:0.05:0.5;
plot(lambdap,blocksu(1,:),'-+'); 
hold on;
plot(lambdap,blocksu(2,:),'r-*');
hold on;
plot(lambdap,blocksu(3,:),'o-');
legend('N=3','N=5','N=7');
xlabel('Lamdap');
ylabel('SU block probability');
title 'SU block probability with lamdap';%SU用户在lamdap下的阻塞概率
grid on; 
hold off;
